...
def direct(lon1,lat1,azimuth,distance):
    ...
    @param lon1,lat1: Position P1 in degrees
    @param azimuth: Clockwise angle in degrees relative to true north.
    @param distance: Distance in meters
    @return: Position P2 in degrees (lon2,lat2)
    '''
...
def inverse(lon1,lat1,lon2,lat2):
    ...
    azimuth = math.degrees(alpha1)
    if azimuth < 0.0:
        azimuth += 360.0

    return azimuth,s

def testDirect(lon1,lat1,azimuth,distance):
    print 'p1',lon1,lat1,'azimuth',azimuth,'distance',distance
    p2 = direct(lon1,lat1,azimuth,distance)
    print 'direct',p2
    a,d = inverse(lon1,lat1,p2[0],p2[1])
    print 'inverse','azimuth',a,'distance',d
    print

def testInverse(lon1,lat1,lon2,lat2):
    print 'p1',lon1,lat1,'p2',lon2,lat2
    a,d = inverse(lon1,lat1,lon2,lat2)
    print 'inverse','azimuth',a,'distance',d
    p2 = direct(lon1,lat1,a,d)
    print 'direct',p2
    print
...
if __name__=='__main__':
    testDirect(-70.0,43.0,45.0,5000.0)
    testDirect(-70.0,43.0,0.0,5000.0)
